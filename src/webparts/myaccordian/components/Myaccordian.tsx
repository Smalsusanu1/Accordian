...
import { IMyaccordianState } from "./IMyaccordianState";
import IAccordionListItem from "../models/IAccordionListItem";
import { WebPartTitle } from "@pnp/spfx-controls-react/lib/WebPartTitle";
import './accordion.css';


export default class Myaccordian extends React.Component<IMyaccordianProps,IMyaccordianState, {}> {
  constructor(props: IMyaccordianProps, state: IMyaccordianState) {
    super(props);
    this.state = {
  ...
  private listNotConfigured(props: IMyaccordianProps): boolean {
    return props.listName === undefined ||
      props.listName === null ||
      props.listName.length === 0;
  }

  private searchTextChange(event:any) {
  ...
  private readItems(): void {
    ...
    const restAPI = `https://smalsusinfolabs.sharepoint.com/sites/Dashboard/Anubhav/_api/lists/getbyid('7abd66f6-8c75-46bf-ae12-fb6cc05effb9')/items` +
      `$${selects}` +
  ...
  public render(): React.ReactElement<IMyaccordianProps> {
    ...
    const pagedItems: JSX.Element[] = this.state.pagedItems.map((item: IAccordionListItem, i: number): JSX.Element => {
      return (
        <AccordionItem>
          <AccordionItem className="accordion__title">
          ...
          <AccordionItem className="accordion__body">
